...
def build_hogfield_weights(pattern_pm1):
    """
    Build a Hogfield weight matrix W from a single pattern in {+1, -1}.
...
def hogfield_converge(initial_state_pm1, W, max_iter=20):
    """
    Synchronously update the Hogfield network until stable or max_iter.
...
#             to Converge in a Hogfield Network
# ----------------------------------------------------------------------
...
class HogfieldGA:
    def __init__(self, rows=8, cols=8, population_size=20, mutation_rate=0.02):
        ...
        # We'll build the Hogfield weight matrix from this target pattern
        target_pm1 = bits_to_pm1(self.target_bits.flatten())
        self.W = build_hogfield_weights(target_pm1)  # shape (n, n)
    ...
    def evaluate_fitness(self, individual):
        """
        Evaluate how close the Hogfield network's stable state is to the target pattern.
        ...
        # Run Hogfield
        stable_pm1 = hogfield_converge(init_pm1, self.W, max_iter=20)
    ...
    def hogfield_converged_of_best(self):
        """
        Return the Hogfield stable 2D bit pattern of the best individual.
        That is, we take best_individual, run it through Hogfield convergence, 
        ...
        stable_pm1 = hogfield_converge(init_pm1, self.W)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("GA + Hogfield Convergence (Toy Example)")
        ...
        # Two Pattern2DWidget's: left = best individual's bits, right = Hogfield-converged
    ...
    def handle_next_generation(self):
        """
        Run one GA generation, then update the display.
        """
    ...
    def update_display(self):
        ...
        Update the two 2D widgets with:
          ...
          2) Its Hogfield-converged stable pattern
        ...
        # Show its Hogfield converged pattern
        converged = self.ga.hogfield_converged_of_best()
